...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchStatus() {
        apiRequest('/api/v1/status')
            .then(data => {
                logMessage('Status fetched successfully via REST.');
                updateUI(data);
            })
            .catch(error => {
                logMessage(`Error fetching status via REST: ${error}`);
            });
    }
    ...
    // --- Initial Load ---
    logMessage('UI Initialized. Fetching initial status...');
    fetchStatus(); // Explicitly fetch status on page load
